...
<vc-layouts.main>
    <form action="/page/" method="post" class="mx-auto max-w-2xl">
        ...
        <input type="text" name="name" required autofocus
               placeholder="Give it a name…" value="{{ $pageData['name'] ?? '' }}"
        ...
        <textarea name="description" rows="10"
                  ...
                  class="mt-8 w-full resize-y border border-line bg-surface px-4 py-3 leading-relaxed text-ink/85 text-content-text outline-none transition-colors placeholder:text-ink/30 focus:border-accent">{{ $pageData['description'] ?? '' }}</textarea>
